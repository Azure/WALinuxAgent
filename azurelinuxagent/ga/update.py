...
class UpdateHandler(object):
    ...
    def run(self):
        ...
        # Check if the GoalState incarnation has updated
        updated = self.check_for_update()
        if not updated:
            return 

        # Assume using the current agent (which None causes to be used)
        latest_agent = None

        # TODO: Enable autoupdate -- force disable for now
    ...
    def get_latest_agent(self):
        available_agent = [agent for agent in self.agents if not agent.err.is_blacklisted()]
    ...
    def check_for_update(self):
        """
        1. Get latest version not in black list
        2. Return True / False if the incarnation has changed (regardless if the agent is updated)
        """
        self.agents = []
        try:
        ...
        manifests = [manifest for manifest in manifest_list.vmAgentManifests if manifest.family == family]
        if len(manifests) == 0:
            message = u"No available guest agent found for: {0}".format(family)
        ...
        # Only consider versions more recent than current
        pkgs = [pkg for pkg in pkg_list.versions if Version(pkg.version) > Version(AGENT_VERSION)]
        ...
        # Purge from the error list records for non-existent agents
        self.error_record = {}
