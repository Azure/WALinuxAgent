...
class RemoteAccessHandler(object):
    ...
    def handle_remote_access(self):
        ...
        existing_jit_users = set(
            u[0] for u in all_users if self.validate_jit_user(
                u[4]))
        self.err_message = ""
        if self.remote_access is not None:
            goal_state_users = set(
                u.name for u in self.remote_access.user_list.users)
            for acc in self.remote_access.user_list.users:
                try:
                    ...
                    self.err_message = self.err_message + \
                        "Error processing user {0}. Exception: {1}" .format(acc.name, ustr(rae))
            for user in existing_jit_users:
                try:
                    ...
                    self.err_message = self.err_message + \
                        "Error removing user {0}. Exception: {1}" .format(user, ustr(rae))
        else:
            # All users removed, remove any remaining JIT accounts.
            for user in existing_jit_users:
                try:
                    logger.info(
                        "User {0} removed from remote_access. remote_access empty".format(user))
                    self.remove_user(user)
                except RemoteAccessError as rae:
                    self.err_message = self.err_message + \
                        "Error removing user {0}. Exception: {1}" .format(user, ustr(rae))
    ...
    def add_user(self, username, encrypted_password, account_expiration):
        ...
        try:
            ...
            raise RemoteAccessError(
                "Error adding user {0} cleanup successful".format(username), ustr(e))
    ...
    def handle_failed_create(self, username):
        try:
            ...
            raise RemoteAccessError(
                "Failed to clean up after account creation for {0}.".format(username), e)
