...
def ext_signature_validation_enabled():
    ...
    even if this method returns False. Most of these checks are performance workarounds, and customers who explicitly require signature 
    validation accept the performance cost. If the hard requirements for validation (i.e., ConfidentialVMInfo.is_confidential_vm() and 
    openssl_version_supported_for_signature_validation()) are not met, the policy parsing would have failed before any ext installation
    was attempted.
    """
