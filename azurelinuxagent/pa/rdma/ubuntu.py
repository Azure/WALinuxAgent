...
class UbuntuRDMAHandler(RDMAHandler):
    ...
    def install_driver(self):
        ...
        if not nd_version:
            ...
            return
        #strip the trailing .*, we are looking for number like 144
        nd_version = re.sub('\..*', '', nd_version)

        #Check to see if we need to reconfigure driver
        status,module_name = shellutil.run_get_output('modprobe -R hv_network_direct')
        if status != 0:
    ...
    def update_modprobed_conf(self, nd_version):
        ...
        if not os.path.isfile(modprobed_file):
            ...
            logger.error("RDMA: modprobed conf file is required for Ubuntu")
            return
