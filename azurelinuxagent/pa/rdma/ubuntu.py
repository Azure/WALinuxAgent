...
class UbuntuRDMAHandler(RDMAHandler):
    ...
    def install_driver(self):
        ...
        logger.info(
            "RDMA: current RDMA driver %s nd_version %s" %
        ...
        logger.info(
            'RDMA: kernel_version=%s package_version=%s' %
    ...
    def update_modprobed_conf(self, nd_version):
        ...
        if not os.path.isfile(modprobed_file):
            logger.info(
                "RDMA: %s not found, it will be created" %
                modprobed_file)
        else:
        ...
        if r:
            lines = re.sub(
                r'alias hv_network_direct hv_network_direct_\S+',
        ...
        f.close()
        logger.info(
            "RDMA: hv_network_direct alias updated to ND %s" %
            nd_version)
