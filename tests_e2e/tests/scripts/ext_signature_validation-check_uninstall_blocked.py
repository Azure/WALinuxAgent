...
def main():
    ...
    pattern = (r".*Extension will not be processed: failed to uninstall extension '{0}' because policy specifies that extension must be signed, but the installed extension's signature was not previously validated by the agent.*").format(re.escape(args.extension_name))
