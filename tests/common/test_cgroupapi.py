...
class CGroupsApiTestCase(AgentTestCase):
    ...
    def test_create_should_return_a_SystemdCgroupsApi_on_systemd_platforms(
            self):
        with patch("azurelinuxagent.common.cgroupapi.CGroupsApi._is_systemd", return_value=True):
            api = CGroupsApi.create()

        self.assertTrue(isinstance(api, SystemdCgroupsApi))
    ...
    def test_create_should_return_a_FileSystemCgroupsApi_on_non_systemd_platforms(
        ...
        self.assertTrue(isinstance(api, FileSystemCgroupsApi))

    def test_is_systemd_should_return_true_when_systemd_manages_current_process(
            self):
    ...
    def test_foreach_controller_should_execute_operation_on_all_mounted_controllers(
        ...
        # The setUp method mocks
        # azurelinuxagent.common.cgroupapi.CGROUPS_FILE_SYSTEM_ROOT to have the
        # cpu and memory controllers mounted
        self.assertIn(
            'cpu',
            executed_controllers,
        ...
        self.assertEqual(
            ...
            2,
            'The operation was not executed on unexpected controllers: {0}'.format(executed_controllers))
    ...
    def test_foreach_controller_should_handle_errors_in_individual_controllers(
        ...
        with patch("azurelinuxagent.common.cgroupapi.logger.warn") as mock_logger_warn:
            CGroupsApi._foreach_controller(
                controller_operation, 'A dummy message')
            ...
            self.assertIn(
                ...
                'The operation was not executed on the memory controller')
            self.assertEqual(
                len(successful_controllers),
                1,
                'The operation was not executed on unexpected controllers: {0}'.format(successful_controllers))

            args, kwargs = mock_logger_warn.call_args
            message = args[0]
            self.assertIn(
                'Error in cgroup controller "cpu": A test exception.',
                message)
...
class FileSystemCgroupsApiTestCase(AgentTestCase):
    ...
    def test_cleanup_old_cgroups_should_move_daemon_pid_on_all_controllers(
            self, _):
        ...
        old_cpu_cgroup = os.path.join(
            self.cgroups_file_system_root,
            "cpu",
            "WALinuxAgent",
            "WALinuxAgent")
        old_memory_cgroup = os.path.join(
            self.cgroups_file_system_root,
            "memory",
            "WALinuxAgent",
            "WALinuxAgent")
        ...
        new_cpu_cgroup = os.path.join(
            self.cgroups_file_system_root,
            "cpu",
            VM_AGENT_CGROUP_NAME)
        new_memory_cgroup = os.path.join(
            self.cgroups_file_system_root,
            "memory",
            VM_AGENT_CGROUP_NAME)
        ...
        self.assertEquals(
            kwargs['message'],
            'Successfully cleaned up old cgroups in WALinuxAgent/WALinuxAgent.')

    def test_cleanup_old_cgroups_should_report_errors_from_all_controllers_that_failed(
            self):
        # Set up the mock /var/run/waagent.pid file
        daemon_pid = "42"
        daemon_pid_file_tmp = os.path.join(self.tmp_dir, "waagent.pid")
        ...
        self.assertIn(
            "Failed to clean up old cgroups in WALinuxAgent/WALinuxAgent.",
            kwargs['message'])
        self.assertIn(
            "Error in cgroup controller \"cpu\": [Errno 2] No such file or directory",
            kwargs['message'])
        self.assertIn(
            "Error in cgroup controller \"memory\": [Errno 2] No such file or directory",
            kwargs['message'])
    ...
    def test_create_agent_cgroups_should_create_cgroups_on_all_controllers(
        ...
        def assert_cgroup_created(controller):
            ...
            self.assertTrue(
                any(cgroups.name == VM_AGENT_CGROUP_NAME for cgroups in agent_cgroups))
    ...
    def test_create_extension_cgroups_should_create_cgroups_on_all_controllers(
        ...
        def assert_cgroup_created(controller):
            ...
            self.assertTrue(
                any(cgroups.path == cgroup_path for cgroups in extension_cgroups))
            self.assertTrue(os.path.exists(cgroup_path))
    ...
    def test_get_extension_cgroups_should_return_all_cgroups(self):
        ...
        for cgroup in created:
            self.assertTrue(any(retrieved_cgroup.path ==
                                cgroup.path for retrieved_cgroup in retrieved))
    ...
    def test_start_extension_command_should_add_the_child_process_to_the_extension_cgroup(
        ...
        # The expected format of the process output is
        # [stdout]\n{PID}\n\n\n[stderr]\n"
        ...
        try:
            ...
            self.fail(
                "No PID could be extracted from the process output! Error: {0}".format(
                    ustr(e)))
...
@skip_if_predicate_false(
    is_systemd_present,
    "Systemd cgroups API doesn't manage cgroups on systems not using systemd.")
class SystemdCgroupsApiTestCase(AgentTestCase):
    ...
    def assert_cgroups_created(self, extension_cgroups):
        ...
        for cgroup in extension_cgroups:
            ...
            self.assertTrue(
                match is not None,
                "Unexpected path for cgroup: {0}".format(
                    cgroup.path))
        ...
        with tempfile.TemporaryFile(dir=self.tmp_dir, mode="w+b") as stdout:
            with tempfile.TemporaryFile(dir=self.tmp_dir, mode="w+b") as stderr:
                with patch("azurelinuxagent.common.cgroupapi.subprocess.Popen", wraps=subprocess.Popen) \
                    ...
                    # We should have invoked the extension command only once
                    # and succeeded
    ...
    def test_start_extension_command_should_use_fallback_option_if_systemd_fails(
        ...
        def mock_popen(*args, **kwargs):
            ...
            systemd_command = args[0].replace(
                'systemd-run', 'systemd-run syntax_error')
            ...
            with tempfile.TemporaryFile(dir=self.tmp_dir, mode="w+b") as stderr:
                with patch("azurelinuxagent.common.cgroupapi.add_event") as mock_add_event:
                    with patch("azurelinuxagent.common.cgroupapi.subprocess.Popen", side_effect=mock_popen) \
                            as patch_mock_popen:
                        # We expect this call to fail because of the syntax
                        # error
                        extension_cgroups, process_output = SystemdCgroupsApi().start_extension_command(
                            extension_name="Microsoft.Compute.TestExtension-1.2.3",
                            command="date",
                        ...
                        self.assertEquals(
                            'InvokeCommandUsingSystemd', kwargs['op'])

                        # We expect two calls to Popen, first for the
                        # systemd-run call, second for the fallback option
                        ...
                        self.assertNotIn(
                            "systemd-run --unit", second_call_args)
    ...
    def test_start_extension_command_should_use_fallback_option_if_systemd_times_out(
        ...
        # In that case, we will internally recognize the failure due to the
        # non-zero exit code, not as a timeout.
        original_popen = subprocess.Popen
        ...
        with tempfile.TemporaryFile(dir=self.tmp_dir, mode="w+b") as stdout:
            with tempfile.TemporaryFile(dir=self.tmp_dir, mode="w+b") as stderr:
                with patch("azurelinuxagent.common.cgroupapi.subprocess.Popen", side_effect=mock_popen) \
                    ...
                    # We expect two calls to Popen, first for the systemd-run
                    # call, second for the fallback option
                    self.assertEquals(2, patch_mock_popen.call_count)
                    ...
                    self.assertEquals(extension_cgroups, [])
                    self.assertEquals(
                        expected_output.format("success"), process_output)
            ...
            with tempfile.TemporaryFile(dir=self.tmp_dir, mode="w+b") as stderr:
                with patch("azurelinuxagent.common.cgroupapi.subprocess.Popen", wraps=subprocess.Popen) \
                    ...
                    with self.assertRaises(ExtensionError) as context_manager:
                        ...
                        # We should have invoked the extension command only
                        # once, in the systemd-run case
                        self.assertEquals(1, patch_mock_popen.call_count)
                        args = patch_mock_popen.call_args[0][0]
                        self.assertIn("systemd-run --unit", args)
            ...
            with tempfile.TemporaryFile(dir=self.tmp_dir, mode="w+b") as stderr:
                with patch("azurelinuxagent.common.cgroupapi.add_event"):
                    with patch("azurelinuxagent.common.cgroupapi.subprocess.Popen", side_effect=mock_popen):
                        ...
                        self.assertEquals(expected_output.format(
...
'''
        ...
        def assert_cgroup_created(controller):
            expected_cgroup_path = os.path.join(
                self.cgroups_file_system_root,
                controller,
                "system.slice",
                VM_AGENT_CGROUP_NAME)
            ...
            self.assertTrue(
                any(cgroups.path == expected_cgroup_path for cgroups in agent_cgroups))
            self.assertTrue(
                any(cgroups.name == VM_AGENT_CGROUP_NAME for cgroups in agent_cgroups))
