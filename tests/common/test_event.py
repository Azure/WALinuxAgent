...
class TestEvent(AgentTestCase):
    def test_add_event_should_read_container_id_from_process_environment(self):
        ...
        with patch("azurelinuxagent.common.event.EventLogger.save_event", side_effect=patch_save_event):
            ...
            # Container id is set as an environment variable when parsing the
            # goal state
            ...
            # Container id is updated as the goal state changes, both in
            # telemetry event and in environment variables
            new_container_id = "z6d5526c-5ac2-4200-b6e2-56f2b70c5ab2"
            xml_text = load_data("wire/goal_state.xml")
            xml_text_updated = xml_text.replace(
            ...
            # Assert both the environment variable and telemetry event got
            # updated
            self.assertEquals(
                os.environ[event.CONTAINER_ID_ENV_VARIABLE], new_container_id)
    ...
    def test_add_event_should_handle_event_errors(self):
        with patch("azurelinuxagent.common.utils.fileutil.mkdir", side_effect=OSError):
            with patch('azurelinuxagent.common.logger.periodic_error') as mock_logger_periodic_error:
                ...
                self.assertIn(
                    "[EventError] Failed to create events folder",
                    exception_message)
    ...
    def test_should_log_errors_if_failed_operation_and_empty_event_dir(
            self, mock_logger_info, mock_logger_warn, mock_logger_error, mock_reporter):
    ...
    def test_save_event_rollover(self):
        ...
        num_of_events = 999
        # this makes number of events to num_of_events + 1.
        ...
        events = sorted(os.listdir(self.tmp_dir))
        self.assertTrue(
            len(events) == num_of_events,
            "{0} is not equal to {1}".format(
                len(events),
                num_of_events))
        ...
        self.assertTrue(
            len(events) == num_of_events,
            "{0} events found, {1} expected".format(
                len(events),
                num_of_events))
        ...
        with open(first_event) as first_fh:
            first_event_text = first_fh.read()
            self.assertFalse(
                'first event' in first_event_text,
                "'first event' not in {0}".format(first_event_text))
    ...
    def test_save_event_cleanup(self):
        ...
        self.assertTrue(
            len(events) == 2000,
            "{0} events found, 2000 expected".format(
                len(events)))
        add_event('test', message='last event')
        ...
        self.assertTrue(
            len(events) == 1000,
            "{0} events found, 1000 expected".format(
                len(events)))
        first_event = os.path.join(self.tmp_dir, events[0])
        with open(first_event) as first_fh:
            first_event_text = first_fh.read()
