...
class TestCGroupsTelemetry(AgentTestCase):
    ...
    def _assert_cgroup_metrics_equal(
            self, cpu_usage, memory_usage, max_memory_usage):
        for _, cgroup_metric in CGroupsTelemetry._cgroup_metrics.items():
            self.assertListEqual(
                cgroup_metric.get_memory_usage()._data,
                memory_usage)
            self.assertListEqual(
                cgroup_metric.get_max_memory_usage()._data,
                max_memory_usage)
            self.assertListEqual(
                cgroup_metric.get_cpu_usage()._data, cpu_usage)
                ...
                with patch("azurelinuxagent.common.cgroup.CpuCgroup._get_cpu_percent") as patch_get_cpu_percent:
                    with patch("azurelinuxagent.common.cgroup.CGroup.is_active") as patch_is_active:
                        ...
                        for data_count in range(poll_count, 10):
                            ...
                            self.assertEqual(
                                len(CGroupsTelemetry._cgroup_metrics), num_extensions)
                        ...
                        self.assertEqual(
                            CGroupsTelemetry._cgroup_metrics.__len__(), num_extensions)
                        self._assert_cgroup_metrics_equal([], [], [])
    ...
    def test_telemetry_polling_to_not_generate_transient_logs_ioerror_file_not_found(
            self, mock_read_file, patch_periodic_warn, *args):
        num_extensions = 1
        for i in range(num_extensions):
            dummy_cpu_cgroup = CGroup.create(
                "dummy_cpu_path_{0}".format(i),
    ...
    def test_telemetry_polling_to_generate_transient_logs_ioerror_permission_denied(
        ...
        for data_count in range(poll_count, 10):
            CGroupsTelemetry.poll_all_tracked()
            self.assertEqual(
                poll_count * expected_count_per_call,
                patch_periodic_warn.call_count)

    @patch("azurelinuxagent.common.osutil.default.DefaultOSUtil._get_proc_stat")
    @patch("azurelinuxagent.common.utils.fileutil.read_file")
    def test_telemetry_polling_to_generate_transient_logs_index_error(
            self, mock_read_file, *args):
        num_extensions = 1
        for i in range(num_extensions):
            dummy_cpu_cgroup = CGroup.create(
                "dummy_cpu_path_{0}".format(i),
        ...
        with patch("azurelinuxagent.common.logger.periodic_warn") as patch_periodic_warn:
            # called only once at start, and then gets removed from the tracked
            # data.
    ...
    def test_telemetry_calculations(self, *args):
        ...
        for i in range(num_extensions):
            ...
            self.assertEqual(
                num_summarization_values, len(
                    collected_metrics[name]["cpu"]["cur_cpu"]))
    ...
    def test_cgroup_is_tracked(self, *args):
        ...
        self.assertFalse(CGroupsTelemetry.is_tracked(
            "not_present_cpu_dummy_path"))
        self.assertFalse(CGroupsTelemetry.is_tracked(
            "not_present_memory_dummy_path"))
    ...
    def test_process_cgroup_metric_with_incorrect_cgroups_mounted(self, *args):
        ...
        with patch("azurelinuxagent.common.cgroup.CpuCgroup.get_cpu_usage") as patch_get_cpu_usage:
            with patch("azurelinuxagent.common.cgroup.MemoryCgroup.get_memory_usage") as patch_get_memory_usage:
                ...
                patch_get_memory_usage.side_effect = Exception(
                    "File not found")
                ...
                with patch("azurelinuxagent.common.cgroup.CGroup.is_active") as patch_is_active:
                    ...
                    for data_count in range(poll_count, 10):
                        ...
                        self.assertEqual(
                            CGroupsTelemetry._cgroup_metrics.__len__(), num_extensions)
                        self._assert_cgroup_metrics_equal(
                            cpu_usage=[current_cpu] * data_count, memory_usage=[], max_memory_usage=[])
            ...
            with patch("azurelinuxagent.common.cgroup.MemoryCgroup.get_memory_usage") as patch_get_memory_usage:
                with patch("azurelinuxagent.common.cgroup.CpuCgroup.get_cpu_usage") as patch_get_cpu_usage:
                    with patch("azurelinuxagent.common.cgroup.CGroup.is_active") as patch_is_active:
                        ...
                        patch_get_cpu_usage.side_effect = Exception(
                            "File not found")
...
'''.format(time_to_wait))
                ...
                self.assertIn(
                    "cur_mem", collected_metrics[extn_name]["memory"])
                self.assertIn(
                    "max_mem", collected_metrics[extn_name]["memory"])
                self.assertEqual(
...
class TestMetric(AgentTestCase):
    ...
    def test_metrics(self):
        ...
        self.assertListEqual(generate_metric_list(test_values), [test_metric.average(
        ), test_metric.min(), test_metric.max(), test_metric.median(), test_metric.count()])
