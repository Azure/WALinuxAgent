...
"
    ...
    def _mock_iptables(version=osutil._IPTABLES_LOCKING_VERSION, destination='168.63.129.16'):
        ...
        set_command(osutil._get_firewall_drop_command(wait, AddFirewallRules.get_check_command(), destination))
        set_command(osutil._get_firewall_drop_command(wait, AddFirewallRules.get_append_command(), destination))
        set_command(osutil._get_firewall_accept_command(wait, AddFirewallRules.get_append_command(), destination, uid))
        set_command(osutil._get_firewall_accept_dns_tcp_request_command(wait, AddFirewallRules.get_insert_command(), destination))
...
''')
    ...
    def test_enable_firewall_should_set_up_the_firewall(self):
        ...
        with TestOSUtil._mock_iptables() as mock_iptables:
            ...
            mock_iptables.set_command(osutil._get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=1)

            success = osutil.DefaultOSUtil().enable_firewall(dst_ip=mock_iptables.destination, uid=mock_iptables.uid)

            drop_check_command = TestOSUtil._command_to_string(osutil._get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination))
            accept_command = TestOSUtil._command_to_string(osutil._get_firewall_accept_command(mock_iptables.wait, AddFirewallRules.get_append_command(), mock_iptables.destination, mock_iptables.uid))
            accept_command_dns_tcp = TestOSUtil._command_to_string(osutil._get_firewall_accept_dns_tcp_request_command(mock_iptables.wait, AddFirewallRules.get_insert_command(), mock_iptables.destination))
            drop_add_command = TestOSUtil._command_to_string(osutil._get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_append_command(), mock_iptables.destination))
            ...
            self.assertEqual(mock_iptables.command_calls[2], accept_command_dns_tcp,
    ...
    def test_enable_firewall_should_not_set_firewall_if_the_drop_rule_exists(self):
        ...
        with TestOSUtil._mock_iptables() as mock_iptables:
            drop_check_command = mock_iptables.set_command(osutil._get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=0)
    ...
    def test_enable_firewall_should_check_for_invalid_iptables_options(self):
        ...
        with TestOSUtil._mock_iptables() as mock_iptables:
            ...
            #      line parameters
            drop_check_command = mock_iptables.set_command(osutil._get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=2)
    ...
    def test_remove_firewall(self):
        ...
        with TestOSUtil._mock_iptables() as mock_iptables:
            ...
            def mock_popen(command, *args, **kwargs):
                command_string = TestOSUtil._command_to_string(command)
                if AddFirewallRules.get_delete_command() in command_string:
            ...
            with patch("azurelinuxagent.common.cgroupapi.subprocess.Popen", side_effect=mock_popen):
                ...
                self.assertIn(delete_accept_dns_tcp_command, delete_commands, "The delete owner accept command was not executed")
                self.assertEqual(delete_commands[delete_accept_dns_tcp_command], 2, "The delete tcp accept non root command should have been executed twice")
                # delete rules >= 2.2.26
