...
faux_status_b64 = base64.b64encode(
    bytes(bytearray(faux_status, encoding='utf-8')))
...
class TestHostPlugin(AgentTestCase):
    ...
    def _compare_data(self, actual, expected):
        # Remove seconds from the timestamps for testing purposes, that level
        # or granularity introduces test flakiness
    ...
    def _validate_hostplugin_args(
            self, args, goal_state, exp_method, exp_url, exp_data):
        args, kwargs = args
    ...
    def assert_ensure_initialized(
        ...
        host = hostplugin.HostPluginProtocol(
            endpoint='ws', container_id='cid', role_config_name='rcf')

        host.is_initialized = False
        patch_http_get.return_value = MockResponse(
            body=response_body, reason='reason', status_code=response_status_code)
    ...
    def test_fallback_channel_503(
            self, patch_update, patch_put, patch_upload, _):
        """
    ...
    def test_fallback_channel_410(
        ...
        self.assertEqual(1,
                         ...
                         "Update goal state unexpected call count")
        self.assertTrue(
            patch_update.call_args[1]['forced'],
            "Update goal state unexpected call count")
    ...
    @patch("azurelinuxagent.common.protocol.wire.WireClient.update_goal_state")
    def test_fallback_channel_failure(
            self, patch_update, patch_put, patch_upload, _):
        """
        ...
        with patch.object(wire.StatusBlob,
            ...
            with patch.object(restutil,
                ...
                with patch.object(wire.HostPluginProtocol,
                    ...
                    #
                    # The agent tries to upload using a direct connection, and
                    # that succeeds.
                    self.assertEqual(
                        1, wire_protocol_client.status_blob.upload.call_count)
                    # The agent never touches the default protocol is this code
                    # path, so no change.
                    self.assertFalse(
                    ...
                    self.assertEqual(
                        'ReportStatus', patch_add_event.call_args[1]['op'])
                    self.assertTrue(
                        'Falling back to direct' in patch_add_event.call_args[1]['message'])
                    self.assertEqual(
    ...
    def test_validate_page_blobs(self):
        ...
        page_status = bytearray(
            status_blob.data.ljust(page_size),
            encoding='utf-8')
    ...
    @patch("azurelinuxagent.common.protocol.healthservice.HealthService.report_host_plugin_versions")
    def test_ensure_health_service_called(
            self, patch_http_get, patch_report_versions):
        host_plugin = self._init_host()
    ...
    def test_put_status_healthy_signal(
            self, patch_http_put, patch_http_post, patch_http_get):
        host_plugin = self._init_host()
    ...
    @patch("azurelinuxagent.common.utils.restutil.http_post")
    @patch("azurelinuxagent.common.utils.restutil.http_put")
    def test_put_status_unhealthy_signal_transient(
            self, patch_http_put, patch_http_post, patch_http_get):
        host_plugin = self._init_host()
        status_blob = self._init_status_blob()
        # get_api_versions
    ...
    @patch("azurelinuxagent.common.utils.restutil.http_post")
    @patch("azurelinuxagent.common.utils.restutil.http_put")
    def test_put_status_unhealthy_signal_permanent(
            self, patch_http_put, patch_http_post, patch_http_get):
        host_plugin = self._init_host()
        status_blob = self._init_status_blob()
        # get_api_versions
    ...
    def test_report_fetch_health(
            self, patch_report_artifact, patch_should_report):
        host_plugin = self._init_host()
        ...
        host_plugin.report_fetch_health(
            uri='http://169.254.169.254/extensionArtifact',
            is_healthy=True)
        self.assertEqual(0, patch_should_report.call_count)
    ...
    def test_report_status_health(
            self, patch_report_status, patch_should_report):
        host_plugin = self._init_host()
...
class MockResponse:
    ...
    def read(self):
        return self.body if sys.version_info[0] == 2 else bytes(
