...
from azurelinuxagent.common.protocol.wire import WireProtocol, WireClient, GoalState, ExtensionsConfig, \
    InVMArtifactsProfile, VMAgentManifestUri, StatusBlob, VMStatus, ExtHandlerVersionUri, \
...
class MultiThreadGoalStateTestCase(GoalStateTestCase):
    def setUp(self):
        super(MultiThreadGoalStateTestCase, self).setUp()
        self.mock_data.reload()  # start each iteration of the test with fresh mock data
        self.errors = Queue()

    def _execute_in_multithread_env(self, t1_func, t2_func, t1_args, t2_args, sleep=None):
        t1 = Thread(target=t1_func, args=t1_args)
        t2 = Thread(target=t2_func, args=t2_args)

        t1.start()
        if sleep:
            # Adding some delay between threads to give time for operation to complete
            time.sleep(sleep)
        t2.start()
        t1.join()
        t2.join()

        # In multi-threaded environment, if the child thread throws an error, it will not be caught by the main thread
        # as its a separate heap. To catch those issues added an error Queue to capture any exceptions,
        # print the errors  and raise in Main Thread
        errors = []
    ...
    def test_it_should_handle_multiple_thread_updates_concurrently(self):

        def update_gs(new_incarnation, new_container_id):
            try:
                self.mock_data.set_incarnation(new_incarnation)
                self.mock_data.set_container_id(new_container_id)
        ...
        # Adding a very minor sleep to ensure the order in which the new goal states would be called
        self._execute_in_multithread_env(update_gs, update_gs, (inc1, cont1), (inc2, cont2), sleep=0.001)

        # Since we're forcing the 2nd goalstate later, the incarnation and container Id should match with the 2nd one
        self.assertEqual(self.wire_client.get_goal_state().incarnation, inc2, "Incarnation not same")
        self.assertEqual(self.wire_client.get_host_plugin().container_id, cont2, "Container Id not same")

    def test_goal_state_data_should_be_updated_in_other_thread(self):
        new_incarnation = str(uuid.uuid4())
        new_container_id = str(uuid.uuid4())

        def update_gs(incarnation, container_id):
            try:
                self.mock_data.set_incarnation(incarnation)
                self.mock_data.set_container_id(container_id)
                self.wire_client.update_goal_state()
            except Exception as e:
                self.errors.put(str(e))

        def assert_new_gs(incarnation, container_id):
            try:
                self.assertEqual(self.wire_client.get_goal_state().incarnation, incarnation, "Incarnation mismatch")
                self.assertEqual(self.wire_client.get_host_plugin().container_id, container_id, "ContainerId mismatch")
            except Exception as e:
                self.errors.put(str(e))
                raise

        # Adding a very minor sleep to ensure that assert_new_gs is called after update_gs has started
        self._execute_in_multithread_env(update_gs, assert_new_gs, (new_incarnation, new_container_id),
                                         (new_incarnation, new_container_id), sleep=0.001)

    def test_host_plugin_should_update_concurrently_on_no_incarnation_change(self):

        new_container_id = str(uuid.uuid4())
        new_role_config_name = str(uuid.uuid4())

        def update_host_plugin(container_id, role_config_name):
            try:
                self.mock_data.set_container_id(container_id)
                self.mock_data.set_role_config_name(role_config_name)
                self.wire_client.update_host_plugin_from_goal_state()
            except Exception as e:
                self.errors.put(str(e))

        def assert_host_plugin(container_id, role_config_name):
            try:
                self.assertEqual(self.wire_client.host_plugin.container_id, container_id, "ContainerId mismatch")
                self.assertEqual(self.wire_client.host_plugin.role_config_name, role_config_name, "RoleConfigName "
                                                                                                  "mismatch")
            except Exception as e:
                self.errors.put(str(e))
                raise
        ...
        # Adding a very minor sleep to ensure that assert_new_gs is called after update_gs has started
        self._execute_in_multithread_env(t1_func=update_host_plugin,
                                         t2_func=assert_host_plugin,
                                         t1_args=(new_container_id, new_role_config_name),
                                         t2_args=(new_container_id, new_role_config_name),
                                         sleep=0.001)
