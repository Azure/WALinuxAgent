...
class TestWireProtocol(AgentTestCase):
    ...
    def test_report_vm_status(self, *args): # pylint: disable=unused-argument
        ...
        self.assertEqual(json.dumps(v1_vm_status_fast_track), actual.to_json(extensions_fast_track_enabled=True))
    ...
    def test_get_artifacts_profile_should_use_direct_channel_when_host_channel_fails(self):
        def http_get_handler(url, *_, **kwargs):
            if self.is_in_vm_artifacts_profile_request(url):
                protocol.track_url(url)
            if self.is_host_plugin_in_vm_artifacts_profile_request(url, kwargs):
                return HttpError("Exception to fake an error on the HostGAPlugin channel")
            return None
...
class GoalStateConstructionTestCase(AgentTestCase):
    ...
    def test_ext_config_construction_multiple_dependencies(self):
        ...
        with mock_wire_protocol(DATA_FILE_FAST_TRACK_MULTIPLE_DEPENDENCIES) as protocol:
            ...
            self.assertEqual(2, len(ext_conf.ext_handlers.extHandlers))
            self.assertEqual(0, ext_conf.ext_handlers.extHandlers[0].properties.extensions[0].dependencyLevel)
            self.assertEqual(42, ext_conf.ext_handlers.extHandlers[1].properties.extensions[0].dependencyLevel)
