...
class TestRemoteAccess(AgentTestCase):
    def test_parse_remote_access(self):
        ...
        self.assertEquals(1,
                          len(remote_access.user_list.users),
    ...
    def test_update_remote_access_conf_remote_access(self, _1, _2, _3):
        ...
        self.assertEquals(
            1, len(
                protocol.client.remote_access.user_list.users))
        self.assertEquals(
            'testAccount',
            protocol.client.remote_access.user_list.users[0].name)
        self.assertEquals(
            'encryptedPasswordString',
            protocol.client.remote_access.user_list.users[0].encrypted_password)
