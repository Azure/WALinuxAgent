...
class ExtHandlerInstanceTestCase(AgentTestCase):
    def setUp(self):
        ...
        self.extension_directory = os.path.join(
            self.tmp_dir, "extension_directory")
    ...
    def test_rm_ext_handler_dir_should_not_report_an_event_if_a_child_is_removed_asynchronously_while_deleting_the_extension_directory(
        ...
        # The next 2 asserts are checks on the mock itself, in case the
        # implementation of remove_ext_handler changes (mocks may need to be
        # updated then)
