...
class UpdateTestCase(AgentTestCase):
    ...
    def agent_versions(self):
        v = [FlexibleVersion(AGENT_DIR_PATTERN.match(a).group(1)) for a in
             self.agent_dirs()]
        v.sort(reverse=True)
        return v
...
class TestSupported(UpdateTestCase):
    ...
    @patch('platform.linux_distribution',
           return_value=['Ubuntu', '16.10', 'yakkety'])
    @patch('azurelinuxagent.ga.update.datetime')
...
class TestGuestAgent(UpdateTestCase):
    ...
    @patch("azurelinuxagent.ga.update.GuestAgent._ensure_loaded")
    @patch("azurelinuxagent.ga.update.restutil.http_get")
    def test_download_fallback(self, mock_http_get, mock_loaded,
                               mock_downloaded):
        self.remove_agents()
        self.assertFalse(os.path.isdir(self.agent_path))
        ...
        art_uri = URI_FORMAT_GET_EXTENSION_ARTIFACT.format(host_uri,
                                                           HOST_PLUGIN_PORT)
    ...
    @patch("azurelinuxagent.ga.update.GuestAgent._load_manifest",
           side_effect=UpdateError)
    def test_ensure_downloaded_load_manifest_fails(self, mock_manifest,
                                                   mock_unpack, mock_download):
        self.assertFalse(os.path.isdir(self.agent_path))
    ...
    @patch("azurelinuxagent.ga.update.GuestAgent._unpack")
    @patch("azurelinuxagent.ga.update.GuestAgent._load_manifest")
    def test_ensure_download_skips_blacklisted(self, mock_manifest, mock_unpack,
                                               mock_download):
        agent = GuestAgent(path=self.agent_path)
...
class TestUpdate(UpdateTestCase):
    ...
    def test_emit_restart_event_writes_sentinal_file(self):
        self.assertFalse(
            os.path.isfile(self.update_handler._sentinal_file_path()))
    ...
    def test_upgrade_available_handles_missing_family(self):
        extensions_config = ExtensionsConfig(
            load_data("wire/ext_conf_missing_family.xml"))
        ...
        with patch('azurelinuxagent.common.logger.warn') as mock_logger:
            with patch('tests.ga.test_update.ProtocolMock.get_vmagent_pkgs',
                       side_effect=ProtocolError):
                self.assertFalse(self.update_handler._upgrade_available(
    ...
    def test_upgrade_available_includes_old_agents(self):
        ...
        self.assertTrue(
            self._test_upgrade_available(versions=self.agent_versions()))
    ...
    def test_upgrade_available_skips_if_when_no_new_versions(self):
        ...
        self.assertFalse(
            self._test_upgrade_available(base_version=base_version))
        return
    ...
    def _test_ensure_no_orphans(self, invocations=3,
                                interval=ORPHAN_WAIT_INTERVAL, pid_count=0):
        with patch.object(self.update_handler, 'osutil') as mock_util:
            ...
            #   See http://stackoverflow.com/questions/26408941/python-nested
            ...
            with patch('os.getpid', return_value=42):
                with patch('time.sleep', return_value=None) as mock_sleep:
                    self.update_handler._ensure_no_orphans(
                        orphan_wait_interval=interval)
                    for pid_file in pid_files:
                        self.assertFalse(os.path.exists(pid_file))
                    return mock_util.check_pid_alive.call_count, \
    ...
    def test_get_latest_agent(self):
        ...
        latest_agent = self.update_handler.get_latest_agent()
        self.assertEqual(len(get_agents(self.tmp_dir)),
                         len(self.update_handler.agents))
    ...
    def test_get_latest_agent_skips_unavailable(self):
        ...
        latest_version = self.prepare_agents(count=self.agent_count() + 1,
                                             is_available=False)
        latest_path = os.path.join(self.tmp_dir,
                                   "{0}-{1}".format(AGENT_NAME, latest_version))
    ...
    def test_get_pid_files_returns_previous(self):
        for n in range(1250):
            fileutil.write_file(
                os.path.join(self.tmp_dir, str(n) + "_waagent.pid"),
                ustr(n + 1))
        pid_files = self.update_handler._get_pid_files()
    ...
    def test_purge_agents(self):
        ...
        for agent in purged_agents:
            agent_path = os.path.join(self.tmp_dir, "{0}-{1}".format(AGENT_NAME,
                                                                     agent.version))
    ...
    def test_run_latest_defaults_to_current(self):
        ...
        self.assertEqual(args[0], [get_python_cmd(), "-u", sys.argv[0],
                                   "-run-exthandlers"])
        self.assertEqual(True, 'cwd' in kwargs)
                ...
                with open(stderr_path, "w") as stderr:
                    ...
                    try:
                        self._test_run_latest(
                            mock_child=ChildMock(side_effect=faux_logger))
                    finally:
    ...
    def test_run_latest_nonzero_code_marks_failures(self):
        ...
        with patch('azurelinuxagent.ga.update.UpdateHandler.get_latest_agent',
                   return_value=latest_agent):
    ...
    def test_run_latest_exception_does_not_blacklist_if_terminating(self):
        ...
        with patch('azurelinuxagent.ga.update.UpdateHandler.get_latest_agent',
            ...
            self._test_run_latest(mock_child=ChildMock(
                side_effect=Exception("Attempt blacklisting")))
    ...
    def _test_run(self, invocations=1, calls=[call.run()],
        ...
        with patch(
                'azurelinuxagent.ga.exthandlers.get_exthandlers_handler') as \
                mock_handler:
            with patch(
                    'azurelinuxagent.ga.monitor.get_monitor_handler') as \
                    mock_monitor:
                with patch(
                        'azurelinuxagent.ga.env.get_env_handler') as mock_env:
                    with patch('time.sleep',
                               side_effect=iterator) as mock_sleep:
    ...
    def test_write_pid_file(self):
        ...
        with patch('os.getpid', return_value=1112):
            ...
            self.assertEqual("1111_waagent.pid",
                             os.path.basename(pid_files[-1]))
    ...
    def test_write_pid_file_ignores_exceptions(self):
        with patch('azurelinuxagent.common.utils.fileutil.write_file',
                   side_effect=Exception):
            with patch('os.getpid', return_value=42):
                pid_files, pid_file = self.update_handler._write_pid_file()
                self.assertEqual(0, len(pid_files))
