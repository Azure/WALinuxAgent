    ...
    def test_it_should_set_dns_tcp_iptable_if_drop_available_accept_unavailable(self):
        ...
        with TestOSUtil._mock_iptables() as mock_iptables:
            with _get_update_handler(test_data=DATA_FILE) as (update_handler, _):
                ...
                mock_iptables.set_command(osutil.get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=0)
                # non root tcp iptable rule is absent
                mock_iptables.set_command(osutil.get_firewall_accept_dns_tcp_rule(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=1)
                update_handler.run(debug=True)

                drop_check_command = TestOSUtil._command_to_string(osutil.get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination))
                accept_nonroot_tcp_check_command = TestOSUtil._command_to_string(osutil.get_firewall_accept_dns_tcp_rule(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination))
                accept_nonroot_tcp_insert_command = TestOSUtil._command_to_string(osutil.get_firewall_accept_dns_tcp_rule(mock_iptables.wait, AddFirewallRules.get_insert_command(), mock_iptables.destination))

                get_firewall_packets = TestOSUtil._command_to_string(osutil.get_firewall_packets_command(mock_iptables.wait))
                ...
                self.assertEqual(mock_iptables.command_calls[2], accept_nonroot_tcp_insert_command,
    ...
    def test_it_should_not_set_dns_tcp_iptable_if_drop_unavailable(self):
        ...
        with TestOSUtil._mock_iptables() as mock_iptables:
            with _get_update_handler(test_data=DATA_FILE) as (update_handler, _):
                ...
                mock_iptables.set_command(osutil.get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=1)

                update_handler.run(debug=True)

                drop_check_command = TestOSUtil._command_to_string(osutil.get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination))
                get_firewall_packets_command = TestOSUtil._command_to_string(osutil.get_firewall_packets_command(mock_iptables.wait))
    ...
    def test_it_should_not_set_dns_tcp_iptable_if_drop_and_accept_available(self):
        ...
        with TestOSUtil._mock_iptables() as mock_iptables:
            with _get_update_handler(test_data=DATA_FILE) as (update_handler, _):
                ...
                mock_iptables.set_command(osutil.get_firewall_accept_dns_tcp_rule(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=0)
