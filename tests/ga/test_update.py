    ...
    def test_it_should_set_dns_tcp_iptable_if_drop_available_accept_unavailable(self):
        ...
        with TestOSUtil._mock_iptables() as mock_iptables:
            with self._get_update_handler(test_data=DATA_FILE) as (update_handler, _):
                ...
                mock_iptables.set_command(osutil._get_firewall_drop_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=0)
                # non root tcp iptable rule is absent
                mock_iptables.set_command(osutil._get_firewall_accept_dns_tcp_request_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=1)
                update_handler.run(debug=True)
                ...
                accept_nonroot_tcp_check_command = TestOSUtil._command_to_string(osutil._get_firewall_accept_dns_tcp_request_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination))
                get_firewall_packets_command = TestOSUtil._command_to_string(osutil._get_firewall_accept_dns_tcp_request_command(mock_iptables.wait, AddFirewallRules.get_insert_command(), mock_iptables.destination))
    ...
    def test_it_should_not_set_dns_tcp_iptable_if_drop_and_accept_available(self):
        ...
        with TestOSUtil._mock_iptables() as mock_iptables:
            with self._get_update_handler(test_data=DATA_FILE) as (update_handler, _):
                ...
                mock_iptables.set_command(osutil._get_firewall_accept_dns_tcp_request_command(mock_iptables.wait, AddFirewallRules.get_check_command(), mock_iptables.destination), exit_code=0)
...
class ReportStatusTestCase(AgentTestCase):
    ...
    def _mock_update_handler(iterations=1, **kwargs):
        ...
        # and then another True because the current UpdateHanlder implementation 
