...
class TestProvision(AgentTestCase):
    ...
    def test_provision(self, mock_util, distro_name,
    ...
    def test_is_provisioned_is_provisioned(
            self, mock_deprovision, mock_read, mock_isfile):
        ...
        self.assertEqual(
            0, deprovision_handler.run_changed_unique_id.call_count)

    @patch('os.path.isfile', return_value=True)
    @patch('azurelinuxagent.common.utils.fileutil.read_file',
           return_value="B9F3C233-9913-9F42-8EB3-BA656DF32502")
    @patch('azurelinuxagent.pa.deprovision.get_deprovision_handler')
    def test_is_provisioned_not_deprovisioned(
            self, mock_deprovision, mock_read, mock_isfile):
        ...
        self.assertEqual(1, ph.osutil.is_current_instance_id.call_count)
        self.assertEqual(
            1, deprovision_handler.run_changed_unique_id.call_count)
    ...
    def _provision_test(self,
        ...
        if expect_success:
            ...
            self.assertTrue(
                re.match(
                    r'Provisioning succeeded \(\d+\.\d+s\)',
            ...
            self.assertTrue(
                'Failed to validate OVF: ProvisionGuestAgent not found' in positional_args[0])
